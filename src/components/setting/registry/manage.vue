<template>
  ...
  >
    ...
    <el-dialog
      :title="mode === 'create'?'添加':'编辑'"
      :visible.sync="dialogRegistryFormVisible"
      :close-on-click-modal="false"
      custom-class="dialog-style"
      width="35%"
    >
      <el-form ref="registry" :rules="rules" label-width="160px" label-position="left" :model="registry">
        <el-form-item label="默认使用" prop="is_default">
        ...
        <el-form-item
          v-if="registry.reg_provider === 'swr' || registry.reg_provider === 'ecr'"
          :label="providerMap[registry.reg_provider].region"
          prop="region"
        >
          <el-input size="small" clearable v-model="registry.region"></el-input>
        </el-form-item>
        <el-form-item
        ...
        <el-form-item
          v-if="registry.reg_provider && registry.reg_provider !=='ecr'"
          prop="namespace"
          :rules="{ required: true, message: `请输入${providerMap[registry.reg_provider].namespace}`, trigger: ['blur'] }"
        >
          <template slot="label">{{ registry.reg_provider ? providerMap[registry.reg_provider].namespace : 'Namespace' }}</template>
          <el-input size="small" clearable v-model="registry.namespace"></el-input>
        </el-form-item>
        <el-form-item
          v-if="registry.reg_provider"
          :label="providerMap[registry.reg_provider].access_key"
          :rules="{ required: true, message: `请输入 ${providerMap[registry.reg_provider].access_key}`, trigger: ['blur'] }"
          prop="access_key"
        >
          <el-input size="small" clearable v-model="registry.access_key"></el-input>
        </el-form-item>
        <el-form-item
          v-if="registry.reg_provider"
          :label="providerMap[registry.reg_provider].secret_key"
        ...
        <template v-if="registry.advanced_setting.modified">
          ...
          <el-form-item label="TLS 证书内容（公钥）" prop="secret_key" v-if="registry.advanced_setting.enable_tls">
            <el-input
              ...
              placeholder="选填"
              rows="4"
              @input="showTip($event,registry,'input')"
              clearable
              v-model="registry.advanced_setting.tls_cert"
            ></el-input>
          </el-form-item>
        </template>
      </el-form>
    ...
    <div class="section">
      ...
      <div class="registry-list">
        <template>
          <el-table :data="allRegistry" style="width: 100%;">
            ...
            <el-table-column width="150px" label="默认使用">
              <template slot-scope="scope">
                <el-tag size="small" v-if="scope.row.is_default">默认使用</el-tag>
                <span v-else>-</span>
              </template>
            </el-table-column>
            <el-table-column width="220px" label="修改时间">
              <template slot-scope="scope">
...
export default {
  data () {
    return {
      ...
      rules: {
        reg_provider: [
          { required: true, message: '请选择镜像仓库提供商', trigger: ['blur'] }
        ]
      },
      isModify: false
    }
  ...
  methods: {
    showTip (val, row, type) {
      if (
        (type === 'switch' && val !== row.tempSetting.enable_tls) ||
        (type === 'input' && val !== row.tempSetting.tls_cert)
      ) {
        this.isModify = true
      } else {
        this.isModify = false
    ...
    changeProvider (val) {
      this.$nextTick(() => {
        this.registry = {
          namespace: '',
          reg_addr:
            val === 'dockerhub' ? 'https://registry.hub.docker.com' : '',
          access_key: '',
          secret_key: '',
          reg_provider: val,
          region: '',
      ...
      if (action === 'add') {
        this.$refs.registry.validate(valid => {
          if (valid) {
            ...
            if (
              !this.registry.advanced_setting.enable_tls ||
              this.registry.advanced_setting.tls_cert
            ) {
              this.$confirm(
